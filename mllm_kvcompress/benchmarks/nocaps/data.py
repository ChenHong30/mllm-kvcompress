...
BENCHMARK_NAME = "nocaps"
ALIASES = ("nocaps",)
DEFAULT_SOURCE = "HuggingFaceM4/NoCaps"
DEFAULT_SPLIT = "validation"
LOCAL_NAMES = ("nocaps", "NoCaps")
...
def row_to_samples(row: dict[str, Any], index: int, split: str, config: str | None):
    return [caption_sample(row, index, split, config)]


def caption_sample(row: dict[str, Any], index: int, split: str, config: str | None) -> BenchmarkSample:
    refs = as_string_list(
